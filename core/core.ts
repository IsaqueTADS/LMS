...
export class Core {
  ...
  constructor() {
    ...
    this.db = new Database("./core/db/lms.sqlite");
  ...
  handler = async (request: IncomingMessage, response: ServerResponse) => {
    try {
      ...
      if (!matched) throw new RouteError(404, "rota nao econtrada.");
      ...
      if (error instanceof RouteError) {
        console.error(`${error.status} ${error.message} | ${request.url}`);
        response.statusCode = error.status;
        response.setHeader("content-tyoe", "application/problem+json");
        response.end(
